...
public class EmployeeCardService {
    ...
    private void phoneNumberCard(EmployeeCardDTO model) {
        ...
        if (phoneNumberCard.matches() || (model.getPhoneNumber().length() != phoneNumberLength)) {
        } else {
            log.info("неверный телефонный номер" + model.getEmail());
            throw new IllegalArgumentException("неверный телефонный номер: номер содержит не только цыфры или неверная длина");
        }
    }
}
